...
const main = async () => {
    const filepaths = await glob(["./src/exercises/*.ts", "./src/pending/*.ts"], { nodir: true });

    const sets = await Promise.all(
        filepaths.map((filepath) =>
            readFile(filepath, { encoding: "utf8" }).then((file) => {
                try {
                    return { filepath, url: getUrl(filepath), difficulty: getDifficulty(file), tags: getTags(file) };
                } catch (e) {
