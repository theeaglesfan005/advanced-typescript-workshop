...
type DynamicSegment<Fragment extends string> =
  Fragment extends `[${infer Dynamic}]`
    ? Dynamic extends ""
      ? never
...
type CatchAllSegment<Fragment extends string> =
  Fragment extends `[...${infer CatchAll}]`
    ? CatchAll extends `[...${string}]`
      ? never
      : CatchAll
    : never;

type OptionalCatchAllSegment<Fragment extends string> =
  Fragment extends `[[...${infer Optional}]]` ? Optional : never;
